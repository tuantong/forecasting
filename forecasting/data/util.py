...
def find_leading_zeros(col):
    """
    Find the index of first non zero elements from a series
    ...
    nonzero_idx = (col != 0).idxmax()
    return np.where(np.arange(len(col)) < nonzero_idx, True, False)


def remove_leading_zeros(array):
    """
    Remove zero elements from a NumPy array until the first non-zero value.
